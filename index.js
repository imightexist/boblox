...
pkg.xml.parseString(
  pkg.fs.readFileSync("sitemaps/profile.xml").toString(),
  function (e, res) {
...
let obscenity = new pkg.obscenity.RegExpMatcher({
  ...pkg.obscenity.englishDataset.build(),
  ...pkg.obscenity.englishRecommendedTransformers,
});
...
function nFormatter(num) {
  ...
  let product = item
    ? nFixed(num / item.value, 1)
        .replace(regexp, "")
        .concat(item.symbol)
    : "0";
  if (product.length > 5) {
    return item
      ? nFixed(num / item.value, 0)
          .replace(regexp, "")
          .concat(item.symbol)
      : "0";
  } else {
...
function template(req) {
  let doc = new pkg.jsdom.JSDOM(
    pkg.fs.readFileSync("app/template.html").toString(),
  );
  if (req.path != "/") {
    doc.window.document.querySelector(".rbx-navbar-signup").href +=
      "?returnUrl=" + encodeURI(req.path);
  ...
  if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
    if (Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])) {
      doc.window.document.getElementById("nav-logged-in").style.display =
        "flex";
      doc.window.document.querySelector(".navbar-brand").href = "/home";
      doc.window.document.getElementById("login").style.display = "none";
      doc.window.document.getElementById(
        "nav-menu",
      ).parentElement.style.display = "inline-block";
      doc.window.document.getElementById("robux").innerHTML = nFormatter(
        data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].robux,
      );
      doc.window.document.getElementById("tix").innerHTML = nFormatter(
        data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].tix,
      );
      doc.window.document.getElementById("tix-count").innerHTML =
        Intl.NumberFormat().format(
          data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].tix,
        ) + " Tickets";
      doc.window.document.getElementById("robux-count").innerHTML =
        Intl.NumberFormat().format(
          data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].robux,
        ) + " BOBUX";
      doc.window.document.getElementById("signup").style.display = "none";
      doc.window.document.body.innerHTML += pkg.fs
        .readFileSync("app/sidebar.html")
        .toString();
      doc.window.document.getElementById("sidebar-username").innerHTML =
        data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].usernames[0];
      doc.window.document.body.innerHTML += pkg.fs
        .readFileSync("app/chat.html")
        .toString();
      doc.window.document.getElementById("sidebar-profile").href =
        "/users/" + data.sessions[req.cookies[".BOBLOSECURITY"]] + "/profile";
...
express.use("*\w", function (req, res, next) {
  ...
  if (Object.keys(req.headers).includes("cf-connecting-ip")) {
    banned = data.bannedIPs.includes(
      pkg.crypto
        .pbkdf2Sync(
  ...
  if (
    (req.originalUrl.startsWith("/ipban") || banned) &&
    ...
    if (req.method == "GET") {
      ...
      if (Object.keys(req.query).includes("contentOnly")) {
        if (req.query.contentOnly == "true") {
          let doc = new pkg.jsdom.JSDOM(
            pkg.fs.readFileSync("app/404.html").toString(),
          );
          doc.window.document.title = "BOBLOX.collabai.lol";
          doc.window.document.getElementById("messageHeader").innerHTML =
            "Access Denied";
          //log(doc.window.document.getElementById("messageHeader").innerHTML)
          doc.window.document.getElementById("message").innerHTML =
            '403 | Our content monitors have determined that your behavior at BOBLOX has been in violation of our Terms of Service.<br><br>Your IP has been blacklisted.<br><br>If you wish to appeal, please contact us via <a href="https://discord.gg/PtEPwUP8qG" target="_blank" style="font-weight:300" class="text-name">Discord</a>.';
          doc.window.document.querySelector(".action-buttons").style.display =
            "none";
          doc.window.document.getElementById("errorImage").src =
            "/static/403.png";
      ...
      if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
        if (
          Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])
        ) {
      ...
      doc.window.document.getElementById("content").innerHTML = pkg.fs
        .readFileSync("app/404.html")
        .toString();
      doc.window.document.title = "BOBLOX.collabai.lol";
      doc.window.document.getElementById("messageHeader").innerHTML =
        "Access Denied";
...
express.post("/api/signup", function (req, res) {
  ...
  if (
    ...
    if (
      req.body.password.length < config.minPassLen ||
      req.body.password.length > config.maxPassLen ||
      req.body.username.search(/[^\w]/g) != -1 ||
      req.body.username.startsWith("_") ||
      req.body.username.endsWith("_") ||
      req.body.password == req.body.username
    ) {
      res.status(400);
    } /*else if (pkg.badWords.find(function(a){return req.query.username.toLowerCase().includes(a)})!=undefined||Object.keys(pkg.cuss.cuss).find(function(a){return req.query.username.toLowerCase().includes(a)&&pkg.cuss.cuss[a]==2})!=undefined)*/ else if (
      config.badWords.find(function (a) {
        return req.body.username.toLowerCase().includes(a);
      }) != undefined ||
      obscenity.hasMatch(req.body.username.toLowerCase())
    ) {
      res.status(406);
    } else if (
      data.usedUsernames.includes(req.body["username"].toLowerCase())
    ) {
      res.status(409);
    } else {
      try {
        ...
        user.password.keylen = Math.max(
          config.keylen,
          req.body.password.length + user.password.salt.length,
        );
        user.password.key = pkg.crypto
          .pbkdf2Sync(
            req.body.password,
            user.password.salt,
          ...
          .toString(user.password.encoding);
        ...
        sitemaps.profile.urlset.url.push({
          loc:
            "https://boblox.collabai.lol/users/" +
            data.id[req.body.username.toLowerCase()].toString() +
            "/profile",
        });
...
express.post("/api/login", function (req, res) {
  ...
  if (keys.includes("username") && keys.includes("password")) {
    ...
    if (
      ...
      !Object.keys(data.id).includes(req.body["username"].toLowerCase())
    ) {
      res.status(401);
    } else {
      try {
        ...
        //log(data.users[id])
        if (
          data.users[id].password.key !=
          pkg.crypto
            .pbkdf2Sync(
              req.body.password,
              data.users[id].password.salt,
              data.users[id].password.iterations,
              data.users[id].password.keylen,
              data.users[id].password.digest,
            )
            .toString(data.users[id].password.encoding)
        ) {
          res.status(401);
          res.end();
          return;
...
express.get("/api/checkUsername", function (req, res) {
  ...
  if (keys.includes("username")) {
    ...
    if (
      ...
      req.query.username.search(/[^\w]/g) != -1 ||
      req.query.username.startsWith("_") ||
      req.query.username.endsWith("_")
    ) {
      res.status(400);
    } else if (
      config.badWords.find(function (a) {
        return req.query.username.toLowerCase().includes(a);
      }) != undefined ||
      obscenity.hasMatch(req.query.username.toLowerCase())
    ) {
      res.status(406);
...
express.get("/static/*\w", function (req, res) {
  ...
  if (pkg.fs.readdirSync("static").includes(path.replace("/static/", ""))) {
    res.sendFile(
      pkg.path.join(__dirname, "static", path.replace("/static/", "")),
    );
    //res.end()
  } else {
...
express.get("/sitemaps/*\w", function (req, res) {
  ...
  if (pkg.fs.readdirSync("sitemaps").includes(path.replace("/sitemaps/", ""))) {
    res.sendFile(
      pkg.path.join(__dirname, "sitemaps", path.replace("/sitemaps/", "")),
    );
    //res.end()
  } else {
    /*doc.window.document.getElementById("content").innerHTML = pkg.fs.readFileSync("app/404.html").toString()
...
express.get("/admin/console", function (req, res, next) {
  if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
    if (Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])) {
      if (
        Object.keys(
          data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]],
        ).includes("admin")
      ) {
        if (data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].admin) {
          let doc = new pkg.jsdom.JSDOM(
            pkg.fs.readFileSync("app/console.html").toString(),
          );
          let token = pkg.crypto.randomUUID();
          memory.admins[token] = "open";
          doc.window.document
            .querySelector("meta[name=admin]")
...
express.get("/game/*\w", function (req, res) {
  ...
  if (
    pkg.fs
      .readdirSync("Game")
      .includes(path.replace("/game/", "").replace("/Game/", ""))
  ) {
    res.sendFile(
      pkg.path.join(
        __dirname,
        "Game",
        path.replace("/game/", "").replace("/Game/", ""),
      ),
    );
    //res.end()
  } else {
...
express.get("/games", function (req, res) {
  ...
  if (Object.keys(req.query).includes("contentOnly")) {
    if (req.query.contentOnly == "true") {
      ...
      let doc = new pkg.jsdom.JSDOM(
        pkg.fs.readFileSync("app/games.html").toString(),
      );
      if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
        if (
          Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])
          ...
          doc.window.document
            ...
            .classList.add("container-has-sidebar");
        }
      }
      res.send(doc.serialize());
      res.end();
...
express.get("/web/place", function (req, res) {
  ...
  if (Object.keys(req.query).includes("contentOnly")) {
    if (req.query.contentOnly == "true") {
      let doc = new pkg.jsdom.JSDOM(
        pkg.fs.readFileSync("app/place.html").toString(),
      );
      res.send(doc.serialize()).end();
      return;
    }
  ...
  if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
    if (Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])) {
      if (Object.keys(req.query).includes("contentOnly")) {
        if (req.query.contentOnly == "true") {
          let doc = new pkg.jsdom.JSDOM(
            pkg.fs.readFileSync("app/group create.html").toString(),
          );
          res.send(doc.serialize()).end();
          return;
        }
      ...
      let doc = template(req);
      doc.window.document.getElementById("content").innerHTML = pkg.fs
        .readFileSync("app/group create.html")
        .toString();
      res.send(doc.serialize());
      res.end();
      return;
  ...
  if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
    if (Object.keys(data.sessions).includes(req.cookies[".BOBLOSECURITY"])) {
      if (Object.keys(req.query).includes("contentOnly")) {
        if (req.query.contentOnly == "true") {
          let doc = new pkg.jsdom.JSDOM(
            pkg.fs.readFileSync("app/home.html").toString(),
          );
          doc.window.document.getElementById("content-username").innerHTML =
            "Hello, " +
            data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]]
              .usernames[0] +
            "!";
          doc.window.document.getElementById(
            ...
            "/users/" +
            data.sessions[req.cookies[".BOBLOSECURITY"]] +
            "/profile";
          res.send(doc.serialize());
          //res.end()
          return;
      ...
      doc.window.document.getElementById("content").innerHTML = pkg.fs
        .readFileSync("app/home.html")
        .toString();
      doc.window.document.getElementById("content-username").innerHTML =
        "Hello, " +
        data.users[data.sessions[req.cookies[".BOBLOSECURITY"]]].usernames[0] +
        "!";
      doc.window.document.getElementById("content-avatar").parentElement.href =
        "/users/" + data.sessions[req.cookies[".BOBLOSECURITY"]] + "/profile";
      res.send(doc.serialize());
      res.end();
      return;
...
express.get("/users/*\w/profile", function (req, res, next) {
  ...
  if (Object.keys(req.query).includes("contentOnly")) {
    if (req.query.contentOnly == "true") {
      let doc = new pkg.jsdom.JSDOM(
        pkg.fs.readFileSync("app/profile.html").toString(),
      );
      doc.window.document.querySelector("script").innerHTML +=
        '\ndocument.title="' +
        data.users[Number(req.path.split("/")[2])].usernames[0] +
        ' - BOBLOX"';
      doc.window.document.getElementById("profile-username").innerHTML =
        data.users[Number(req.path.split("/")[2])].usernames[0];
      res.send(doc.serialize()).end();
      return;
    }
      ...
      if (Object.keys(req.cookies).includes(".BOBLOSECURITY")) {
        if (
          ...
          doc.window.document.querySelector(
            ".btn-control-md.btn-fixed-width",
          ).href = "/home";
